...
lo = 7
hi = 7
objp = np.zeros((hi*lo,3), np.float32)

#add 2.5 to account for 2.5 cm per square in grid
objp[:,:2] = np.mgrid[0:hi,0:lo].T.reshape(-1,2)*2.5

# Arrays to store object points and image points from all the images.
objpoints = [] # 3d point in real world space
imgpoints = [] # 2d points in image plane.
images = glob.glob('sourabhImages_2/*.jpg')
...
for fname in images:
    ...
    ret, corners = cv2.findChessboardCorners(gray, (hi,lo), None)
    ...
    if ret == True:
        ...
        cv2.drawChessboardCorners(img, (hi,lo), corners2, ret)
...
# undistort
mapx,mapy=cv2.initUndistortRectifyMap(cam_mtx,dist,None,newcam_mtx,(w,h),5)    
undst = cv2.remap(img1,mapx,mapy,cv2.INTER_LINEAR)
x, y, w, h = roi
undst = undst[y:y+h, x:x+w]
cv2.imwrite("Images/undstnew.jpg",undst)
undst = cv2.undistort(img1, cam_mtx, dist, None, newcam_mtx)
x, y, w, h = roi
undst = undst[y:y+h, x:x+w]
cv2.imwrite("Images/undstnw.jpg",undst)
...
# cv2.imwrite("Images/s.bmp", img1)
# cv2.waitKey(5000)      
# cv2.destroyAllWindows()
# cv2.imshow('img1', undst)
# cv2.imwrite("Images/x.bmp", undst)
# cv2.waitKey(5000)      
# cv2.destroyAllWindows()